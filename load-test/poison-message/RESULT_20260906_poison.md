...
## 후속 — 고치고 다시 쟀다 (2026-09-06 오후)

미결정으로 뒀던 "유한값 검사를 넣을지"를 **넣기로 결정**했다. 비용을 따져보니 판단할
것이 없었다 — 포인트당 비교 8회이고, 같은 포인트를 보내는 HTTP 요청 하나가 이 호스트에서
약 0.8초(요청당 고정비, 배치 크기와 거의 무관)다.

`toPoint()`가 유한하지 않은 값을 만나면 **던진다.** 건너뛰고 저장하면 신호가 없지만,
던지면 컨슈머의 레코드 단위 catch가 그 레코드만 DLQ로 보내 **유실이 보이고 되돌릴 수
있는 형태**가 된다.

같은 시나리오를 다시 돌린 결과(`TYPES=infinity`, `evidence/20260906-102946/`):

| | 고치기 전 | 고친 뒤 |
| --- | --- | --- |
| 대조군 | 100 / 100 | 100 / 100 |
| 독성의 종착지 | **저장됨(speed 필드만 소실)** | **DLQ 1건** |
| 운영자가 볼 수 있는 신호 | 없음 | DLQ + `x-dlq-*` 헤더 + 원인 메시지 |

DLQ 레코드의 원인 메시지는 그대로 읽힌다:

```
InfluxDB에 쓸 수 없는 값 — speed=Infinity (유한하지 않은 값은 필드가 조용히 사라진다)
```

**여기서 또 하나가 드러났다.** 새 예외가 DLQ 분류에서 `unknown`(100%)으로 나왔다 —
`InvalidDataAccessApiUsageException`(Spring이 우리 `IllegalArgumentException`을 번역한다)이
`dlq.py`의 분류 목록에 없었다. 되돌려도 값은 그대로 Infinity라 **영구**다.
목록에 추가했다. 분류 목록은 "지금까지 본 예외만 담고 있다"는 한계가 이렇게 또 나타난다 —
새 실패 유형을 만들면 분류도 같이 봐야 한다.

회귀 테스트: `TelemetryRepositoryTest`(speed=Infinity / engine_temp=NaN / gps.lat=-Infinity /
정상값 통과).
...
- ~~Infinity 부분 유실을 막을지 정하지 않았다.~~ → **같은 날 오후에 넣고 재측정했다**
  (위 "후속" 절). 남은 것은 **NaN·-Infinity를 실제 주입해보지 않은 것**이다 —
  단위 테스트로만 확인했고 파이프라인 전 구간으로는 `1e309`(양의 Infinity)만 흘렸다.
- **유형별 1회 실행이다.**
