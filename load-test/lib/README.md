...
- **`set -e`를 쓰지 않는다.** 한 회차가 실패해도 나머지를 돌린다 — 3회 중 1회 실패는
  그 자체가 결과이고(정책의 "실패 횟수"), 거기서 멈추면 변동 폭을 영영 못 잰다.
- 반복 시작 시각을 마크로 남겨, `summarize.sh`가 **이번 반복분만** 집계한다.
  같은 시나리오의 옛 실행이 섞이면 표가 거짓말을 한다.

`summarize.sh`는 `REPEAT_<시각>.md`를 만든다. **평균을 내지 않는다** — 회차별 값을
그대로 늘어놓고 최소·최대·편차만 덧붙인다. 평균은 "3회 중 1회가 크게 튀었다"를
지워버려서 안정성 판단에 쓸 수 없다.

표에는 회차별 `run-id`, git commit과 **dirty 여부**, 판정도 함께 넣는다 —
어떤 회차가 어떤 코드에서 나왔는지 표에서 바로 보여야 한다.
