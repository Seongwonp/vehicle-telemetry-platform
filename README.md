...
### ML 이상 감지 — 다음 작업 (2026-09-03 기준, 우선순위 순)

13차 측정까지의 결과는 `docs/load-test-plan.md` 10~13차 절과 ADR-018에 있다.
아래는 **아직 안 한 것**만 추린 것이다.

| # | 항목 | 왜 / 무엇을 조심할 것 |
|---|------|----------------------|
| 1 | **`FEATURES`에 `throttle_position` 추가** | 13차에서 `throttle_no_response` recall이 6.9%였는데, 원인이 튜닝이 아니라 **모델이 스로틀을 아예 안 보기 때문**이었다(다른 패턴은 53~92%). 가장 효과가 클 개선. ⚠️ `MLAnomalyDetector.get_state()`가 버퍼를 pickle로 저장하므로 피처 수를 늘리면 **기존 Redis 상태와 호환되지 않는다**(5열 버퍼에 6열 행을 붙이게 됨). 상태에 버전을 넣고 불일치 시 폐기하는 처리가 함께 필요하다 |
| 2 | **채점 재측정** | 13차 수치는 시뮬레이터 버그(복합 이상 1.3%가 룰에 잡힘)가 있던 데이터로 낸 것이다. 버그는 `dc5cd54`에서 고쳤으니 오염 없는 수치를 다시 뽑아야 한다. `load-test/anomaly-detector-scale/score_ml.py` 사용 |
...
| 4 | **`contamination` 재설계** | 12차에서 확인했듯 `contamination=0.05`는 탐지율이 아니라 **표시할 비율**이라, 정상만 있어도 항상 5%가 알림으로 뜬다. `score_samples()` + 별도 임계값으로 바꾸거나 알림 집계·억제 계층이 필요하다 |
