...
| 25 | 인스턴스 스윕(1→2→3→4)과 CPU 계측 — 같은 조건에서 **24,390 → 40,470 → 44,771 → 57,864 msg/s로 꺾이지 않고**, InfluxDB CPU도 49%→153%로 함께 오른다. **상한을 아직 못 찾았다**(파티션 12·concurrency 3이라 4개가 스레드 상한) | 완료(1회) |
