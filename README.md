...
> 직전 목록의 1·2번(`throttle_position` 추가, 오염 없는 재채점)은 **완료했다**(14차).
> 진단대로 `throttle_no_response` recall이 6.9% → **99.6%**, 복합 이상 전체가
> 52.1% → **77.4%** 로 올랐다. 다만 **오탐이 함께 커져**(ML 알림 2.4배, 그중 63.6%가
> 정상 메시지) 순이득이 아니었고, 그 결과 아래 우선순위가 바뀌었다.

| # | 항목 | 왜 / 무엇을 조심할 것 |
|---|------|----------------------|
| 1 | **`contamination` 재설계** | `contamination=0.05`는 탐지율이 아니라 **표시할 비율**이라 정상만 있어도 항상 5%가 알림으로 뜬다(12차). 14차에서 **더 시급해졌다** — 피처를 하나 늘리자 recall과 함께 오탐도 늘어(전체 처리 대비 알림률 약 5.3% → 약 12.9%), 고정 비율로 뽑는 구조를 그대로 두면 **피처를 개선할수록 오탐이 따라온다**. `score_samples()` + 별도 임계값으로 바꾸거나 알림 집계·억제 계층이 필요하다 |
| 2 | **14차 오탐 증가 원인 규명** | ML 알림이 2.4배가 되고 두 패턴(`alternator_degrading`, `overheat_at_idle`)은 14~15pp 하락했다. 원인은 **추정만 했고 측정하지 않았다** — (a) 정상 주행에서 스로틀 분산이 커 학습 윈도우와 채점 구간의 분포 차이가 벌어짐, (b) 복합 이상 5%가 학습 버퍼에도 섞여 모델이 오염된 분포를 학습. `COMPOSITE_ANOMALY_RATE=0`으로 한 번 재보면 (b)를 분리할 수 있다 |
| 3 | **재학습 주기 적정성 측정** | `retrain_min_seconds=60`은 **성능만 보고 정한 값**이라 concept drift 대응으로 충분한지 미검증. 시뮬레이터의 `DRIFT_TEMP_DELTA`로 정상 분포를 이동시킨 뒤, 알림률이 원래 수준으로 돌아오는 데 걸리는 시간을 재면 된다(시나리오는 이미 구현돼 있고 아직 안 써봤다) |
| 4 | ML 켠 상태 장기 soak | 11차 기준 ML ON은 유입과 겨우 균형이고 백로그가 안 줄었다. 장시간 안정성은 미확인 |
