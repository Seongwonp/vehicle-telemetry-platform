...
| 36 | **부하 중 스케일 다운** (2회) — 제거된 인스턴스가 쥔 **6개 파티션이 완전히 정지**(평균 lag 7,137 → 177,952)하는 동안 **살아있는 backend의 3개는 평평하다**(2,244 → 1,324). 격리는 성립하고 대가는 그 6개다. 재할당은 무부하 때와 같은 46~52초, **유실 0**(토픽 = InfluxDB 행, 2회 모두 일치). 곁가지로 **"소유자가 있다 ≠ 소비되고 있다"** — 정적 멤버십은 죽은 멤버가 소유권을 유지해서 `--describe`만 보면 멀쩡해 보인다 | 완료(2회) |
