...
| 18 | 장애 정합성 정량화 — 발행 측 계측(PUBACK 기준)으로 **차량 → InfluxDB 전 구간 대조**를 만들고, MQTT 브로커 장애의 72% 유실과 큐 초과 4.7% 유실을 찾아 각각 0으로. DLQ 재처리·리밸런싱 재전달의 멱등성도 실측 | 완료 |
| 19 | 이상 알림 저장 배치화 — 49 msg/s(유입의 1/4)를 유입 전량 소화로. DB 장애 시 DLQ 직행을 재시도 예산으로 바꿔 PostgreSQL 300초 장애에서 유실 0 | 완료 |
| 20 | 장기 장애 경계 측정 — 같은 720초 장애를 의존성만 바꿔 주입해 **재시도 예산이 소진되는 지점**을 처음 관측(InfluxDB 220초 / PostgreSQL 485초). DLQ 발행이 로그에 안 남던 관측 구멍도 수정 | 완료 |
| 21 | 엔드투엔드 추적 — 표식 차량으로 개별 레코드를 MQTT→Kafka→InfluxDB→REST→WebSocket 5단계에서 확인(20/20), WebSocket 지연 p50 536ms | 완료 |
| 22 | 남은 미결정 정리 — 조용히 사라지던 필드(Infinity)를 DLQ로, 리밸런싱이 안 도는 이유를 바이트코드로 규명, 장애 중 추적으로 실패 지점 구분 확인, 죽은 토픽 제거 | 완료 |
| 23 | 저장 경로 수평 확장 경계 — 통째로 복제하면 용량이 안 늘고(멤버 3 그대로, fencing 45) 역할을 나누면 는다(멤버 9, 유실·중복 0). `mqtt.ingest.enabled` 스위치 추가 | 완료 |
| 24 | 수평 확장의 **처리량 이득** 측정 — 파티션 12·Kafka 직접 발행 조건에서 **1개 19,428 → 3개 50,894 msg/s**(A/B/A 대조, 표본 범위 비중첩). 단 **배수는 부하 조건의 함수**로 발행 압력에 따라 1.34~2.62배 | 완료(1회) |
| 25 | 인스턴스 스윕과 CPU 계측 — 파티션 12에서 **24,390 → 40,470 → 44,771 → 57,864 msg/s로 꺾이지 않아** 상한을 못 찾았다 | 완료(1회) |
| 26 | InfluxDB 보존 변경 절차 — 기존 환경은 `bucket update`가 필요. 실측으로 **삭제는 검사 주기마다**(운영 기본 30분), **삭제 중 쓰기 영향 0**, **되돌릴 수 없음**을 확인하고 Runbook화 | 완료(1회) |
| 27 | 파티션 24·인스턴스 8까지 재스윕 — **수확 체감 확인**(+125%/+17%/+17%)하고 **CPU를 범인에서 제외**(호스트 923%/1600%, backend CPU 합 4개→8개에서 142→147 정체) | 완료(2회) |
| 28 | **천장의 원인 규명 — InfluxDB 쓰기 지연.** 배치 크기 2,000 고정인데 지연이 388 → 378 → **665** → 717ms. 1→2는 지연 그대로에 처리량 2배, **2→4에서 지연 +76%와 함께 증가가 +17%로 꺾인다** | 완료(2회) |
| 29 | **3회 반복 — "이 환경에서는 천장을 숫자로 말할 수 없다"가 결론.** 같은 조건 3회에서 모든 인스턴스 수가 **단조 감소**(8개: 56,985 → 44,279 → **26,564**), 3회차에선 8개가 4개보다 20% 느림. 원인 미규명(디스크·호스트 CPU 아님, 발열 스로틀링 의심). **회차 간 비교 불성립** — 살아남는 건 방향뿐 | 완료(3회) |
| 30 | **수집/저장 분리를 compose로** — 실험 스크립트의 `docker run`에만 있던 구성을 프로파일 `scale`로 옮겼다. 인스턴스 3개(수집 1 + 저장 2)가 파티션 9개를 정확히 3개씩, 정적 id 9종 무충돌, fencing 0, MQTT 세션은 주 backend 하나. Prometheus는 `file_sd_configs`로 있을 때만 긁는다. **대가를 새로 쟀다 — 스케일 다운은 재할당까지 약 45초**(정적 멤버십의 `session.timeout.ms`) | 완료(구조 1회) |
| 31 | **`session.timeout.ms`를 명시로 고정** — 어제 측정한 "스케일 다운 45초"가 **설정 어디에도 없는 클라이언트 기본값**에 매달려 있었다(P0-3의 반대 방향: 설정이 있는데 동작이 없던 것 ↔ 동작이 있는데 설정이 없던 것). 같은 값(45,000ms)을 명시해 동작 변화 0. 회귀는 실제 `application.yml`을 읽는 계약 테스트 3건 | 완료 |
| 32 | **dev(평문) 프로파일 `scale` 실기동** — 구조는 mTLS 프로파일과 동일(멤버 9 / 정적 id 9 / 할당 9, fencing 0). **파티션을 늘려도 이미 떠 있는 컨슈머는 `metadata.max.age.ms`(기본 5분)까지 못 본다**는 것을 새로 관찰(실측 4분 27초) — "인스턴스를 늘렸는데 처리량이 그대로"로 보이는 상태다. 스케일 다운 2회차 46초 | 완료(구조 1회) |
| 33 | **NaN·음의 Infinity 주입** — `-1e309`는 `toPoint()`의 유한값 검사에 걸려 DLQ(양수와 같은 예외·분류). **`NaN`은 그 검사까지 오지도 않는다** — JSON 리터럴이 아니라 역직렬화 단계에서 거부된다. 두 유형 모두 대조군 100/100 생존. "검사가 있으니 안전하다"와 "그 경로로 못 들어온다"를 구분해 기록 | 완료(각 1회) |
| 34 | **스케일 다운 3회 반복** — dev 프로파일 동일 조건 **46/48/47초**(폭 2초, 4.3%). 프로젝트 기준(3회)을 채워 "약 45~48초"까지 말할 수 있다. 측정 중 4회차 하나를 버렸다(대기 조건이 `할당>=9`뿐이라 저장 인스턴스가 붙기 전에 통과) | 완료(3회) |
| 35 | **정수 필드(`rpm`) 경계** — 범위 초과와 `1e309`는 **역직렬화**에서 DLQ(같은 `1e309`가 `speed`에서는 `finite()`까지 온다 — **막는 문이 타입에 따라 다르다**). 그런데 **`rpm: 2000.7`은 아무 데서도 안 걸리고 2000으로 잘려 저장된다**(DLQ·에러·카운터 0, 행 수도 맞아 정합성 대조로도 안 잡힘). 손실이 1 rpm 미만이고 이상 감지는 원본 JSON을 보므로 **고치지 않기로 결정** | 완료(각 1회) |
| 36 | **부하 중 스케일 다운** (3회) — 제거된 인스턴스가 쥔 **6개 파티션이 완전히 정지**(평균 lag 7,137 → 177,952)하는 동안 **살아있는 backend의 3개는 평평하다**(2,244 → 1,324). 격리는 성립하고 대가는 그 6개다. 재할당은 무부하 때와 같은 46~54초, **유실 0**(토픽 = InfluxDB 행, **3회 모두 일치**). 곁가지로 **"소유자가 있다 ≠ 소비되고 있다"** — 정적 멤버십은 죽은 멤버가 소유권을 유지해서 `--describe`만 보면 멀쩡해 보인다 | 완료(3회) |
| 37 | **정적 멤버십 대조군** — 끄니 재할당이 **다운 후 9초**(켠 상태 46~54초), **정지 파티션 0개**(켠 상태 6개). 스케일 다운의 45초는 정적 멤버십의 대가가 맞다. 끄려면 설정이 필요해 `KafkaConfig`에 "빈 값 = 끄기"를 넣었다(Kafka가 빈 문자열을 거부해 그전엔 **끌 방법 자체가 없었다**). **⚠ 끄는 것의 대가(리밸런싱 폭풍)는 미측정** — 기본값은 켜짐 유지 | 켠 쪽 3회 / 끈 쪽 1회 |
| 38 | **`dtc_codes`(배열) 경계** — 배열이 아니면 DLQ. 그런데 **`[null]`은 문자열 `"null"`로, 쉼표 든 코드는 구분자와 충돌**해 저장된다(둘 다 신호 0). 값이 사라지진 않고 스펙 위반 입력에서만 생겨 **고치지 않기로 결정**, 다시 볼 조건을 코드 주석에 남김 | 완료(각 1회) |
| 39 | **`session.timeout.ms`를 줄였을 때의 대가** — `docker pause`로 **같은 20초 정지**를 주입: 45,000ms는 리밸런싱 **0회**(멤버 6 유지), 10,000ms는 **2회**(정지 중 멤버 6 → 3). 스케일 다운은 계획할 수 있지만 GC 정지·순단은 고를 수 없어 **45,000ms 유지**로 결정. 양쪽 대가가 처음으로 다 숫자가 됐다 | 완료(각 1회) |
| 40 | **정적 멤버십을 끄는 쪽의 대가** — 330초 정지(`max.poll.interval.ms` 초과)에서 재가입 로그의 **`MemberIdRequiredException`이 ON 0건 / OFF 12건**(12시간 soak test에서 본 그 예외). 끄면 컨슈머 전부가 member id를 재발급받는다. **폭풍 자체는 재현 못 함** — 전부 정상 재가입. 곁가지로 **내 집계가 틀렸다**(정지시키지 않은 컨테이너를 세고, 정상 리밸런싱에도 찍히는 줄을 표식에 넣어 "차이 없음"이 나왔다 — 원본 로그로 잡음) | 완료(각 1회) |
