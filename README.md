...
| 28 | **천장의 원인 규명 — InfluxDB 쓰기 지연.** 배치 크기 2,000 고정인데 지연이 388 → 378 → **665** → 717ms. 1→2는 지연 그대로에 처리량 2배, **2→4에서 지연 +76%와 함께 증가가 +17%로 꺾인다.** 같은 실행이 앞선 "약 57,000" 추정도 정정(2회가 25% 차이) | 완료(1회) |
