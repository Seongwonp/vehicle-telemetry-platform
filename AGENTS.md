...
- **증거 우선**: 기능 추가보다 유실·중복·순서·장애 복구 주장의 재현성을 먼저 높인다.
- **실제 OBD-II 연동 고려**: 시뮬레이터와 실제 OBD-II 동글 전환이 쉽도록 인터페이스 분리
- **포트폴리오용**: 코드 가독성, README, 아키텍처 문서 품질 중요
- **현재 기준 문서**: `docs/current-state-audit-2026-09-09.md`, `docs/roadmap.md`,
  `docs/evidence-policy.md` 순으로 확인한다.
- Kubernetes, 서비스 추가 분리, 신규 저장소는 현재 문제와 측정 근거가 있을 때만 검토한다.
...
- 커밋과 푸시는 사용자가 명시적으로 요청한 경우에만 직접 실행할 것
