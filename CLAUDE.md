...
### 3. P1 — Redis 장애 정책 — **3a 구현·단일 장애 확인 완료 / 3b 부하 중 동작 미검증**

완료 범위의 기준은 `docs/roadmap.md` 3a·3b다. **조회 2초·진단 4초는 무부하 단건 관찰**이라
스레드 고갈의 근거로 쓰지 않고, 회로 차단기는 3b 측정 전에 추가하지 않는다.
2026-09-13에 readiness에서 redis를 뺐다 — 넣으면 fail-open 경로를 라우팅이 닫는다(§9-1).

같은 날 오후(`docs/redis-failure-policy.md` §10): **알림 규칙을 "최근 5분 이력"에서 "진행 중"으로 바꿨다**(합계 카운터 기동 시 0 등록 +
`[1m]`, `for: 1m` 유지) — 90초 장애는 실패 중 firing, 해제 약 1분이 됐지만 **30초 장애는 발송 0건**이 됐다.
**이 한계는 수용했다** — 두 알림은 **"요청 처리에서 관찰한 Redis 영향"** 알림이지 Redis 가용성 감시가 아니다. 이력 알림·Alertmanager 주기 변경 없음,
요청이 없을 때의 탐지는 백로그. 90초에서도 firing 수신은 Redis 응답 뒤였다(§10-4).
회복 지연은 30/90초 각 3회에서 앱 회복이 **Lettuce 재연결 로그와 겹쳤고 재연결 대기가 주요 후보**다(원인 확정 아님).
감지기 DLQ의 `x-dlq-replay-count` 누락을 재현·수정했다.
