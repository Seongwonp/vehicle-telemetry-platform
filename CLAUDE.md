...
> 회차 표: `REPEAT_20260907_120453.md`.
>
...
>
> **`summarize.sh`도 고쳤다** — `$((mx - mn))`이 정수 산술이라 **소수 지표에서 문서
> 생성이 조용히 끊겼다**(표가 한 줄에서 끝남). awk로 바꿨다.
> **오늘 조용한 실패가 셋**이었고 전부 `set -euo pipefail`이나 리다이렉트와 겹쳐
> 에러가 안 보였다.
>
