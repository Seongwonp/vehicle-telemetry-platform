...
>
> **다음 후보**: 앱 **실기기 검증**(기기 필요 — 유일하게 남은 P1 항목),
> split 구성을 compose에 반영(기존 스크립트가 `telemetry-backend` 이름·8080에 의존),
> 처리량 상한의 원인 규명(InfluxDB인지 호스트 CPU인지 — 인스턴스 2·6개와 CPU 계측 필요),
> 정적 멤버십을 끈 대조군,
> `FAULT=telemetry-kafka` 등 다른 단계에서의 추적, 3회 반복(현재 대부분 1~2회),
> NaN·-Infinity 파이프라인 주입, 데이터 보존·삭제 정책(GPS는 위치정보).
> 서술 기록은 `docs/devlog.md`의 2026-09-06 항목.
