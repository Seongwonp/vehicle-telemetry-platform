...
**배경**: ADR-015에서 확인한 진짜 병목 — 단일 인스턴스 `anomaly-detector-group`이 ~2,500 msg/s
지속 부하에서 lag이 1,264 → 200만 건 이상으로 폭주했다. 다만 그 테스트는 시뮬레이터 프로세스
6개가 같은 8코어 호스트를 같이 썼기 때문에, "서비스 자체의 처리 한계"와 "호스트 CPU를
시뮬레이터한테 뺏겨서 느려 보인 것"이 섞여 있었다. 고치기 전에 먼저 원인을 분리했다.

**0단계 실측(원인 분리)**: 시뮬레이터를 2프로세스(~1,200 msg/s, 호스트 여유 있음)로 줄여
12분 관찰 — lag이 수천 단위에서 진동만 하고 마지막엔 231까지 떨어졌다(거의 다 따라잡음).
즉 ~1,200 msg/s에서는 단일 인스턴스도 평균적으로는 감당한다. 원래 폭주(~2,500 msg/s)와
비교하면, 진짜 처리 한계는 이 둘 사이 어딘가에 있고 호스트 경합이 폭주를 더 키운 것으로 보인다.
...
(고정 이름과 `--scale`은 공존 불가), `docker compose up -d --scale anomaly-detector=3`으로
파티션 수(3)에 맞춰 3개 인스턴스를 띄운다. 로그 포맷에 컨테이너 hostname을 추가해
여러 인스턴스의 로그를 구분할 수 있게 했다(`anomaly_detector.py`).
...
(`ML_ENABLED=true`로 IsolationForest를 켤 경우, 인스턴스마다 자기 파티션에 들어오는
차량들로만 개별 학습하게 된다는 점은 알아둬야 한다 — 현재 기본값은 `false`라 이번 결정에서는
문제가 되지 않지만, ML을 켤 계획이 생기면 짚고 넘어가야 할 트레이드오프다.)

**재측정 결과**: 원래 폭주를 냈던 것과 같은 부하(6프로세스, ~2,324 msg/s)로 11분 재실행.
단일 인스턴스는 lag이 무한정 쌓였지만, 3인스턴스는 16,438 → 20,690 → ... → 7,368로
(끝으로 갈수록 오히려 낮아지며) 수천~2만 건대에서 진동만 하고 발산하지 않았다. Storage
경로(Java→InfluxDB) lag은 이 전 과정에서 계속 건강했다(수백 단위).

**한계**: 11분 관찰이라 "영원히 안 쌓인다"까지 단정하긴 이르다(원래 24시간 테스트와 비교하면
관찰 시간이 짧다). 다만 원래 테스트의 발산 조짐은 몇 분 안에 뚜렷했던 것과 달리 이번엔 그런
조짐이 전혀 없었다는 점에서, 실용적으로는 충분히 신뢰할 수 있는 결과로 판단했다.

**부수적으로 발견한 것**: 재빌드 과정에서 `kafka-python`이 (버전 미고정으로) 3.0.7→3.0.9로
올라갔다 — 재현성 관점에서 버전 고정을 고려할 만하다. 인스턴스 시작 시 `kafka.net.selector`가
이벤트 루프 블로킹 경고를 1회 냈지만 그룹 조인 시점의 1회성이라 지속적인 처리 지연의
원인은 아닌 것으로 판단했다.

**상세 수치**: `docs/load-test-plan.md` 5절 "Track A — 이상 감지 서비스 다중화".
