...
>
> 다만 **최소 필요 인스턴스 수는 여전히 열린 질문이다** — 1인스턴스가 88%를 처리한다는
> 건 "3배 필요"가 아니라 "1.14배 부족"이라는 뜻이라 2인스턴스로도 충분할 수 있다.
> 파티션 수와 정렬한다는 이유로 3을 유지하되, 측정은 하지 않았음을 밝혀둔다.
> 장기(수 시간) 안정성도 이번 관찰(arm당 4-5분)로는 확정하지 않았다.
>
