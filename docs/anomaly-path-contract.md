...
> 상태: **조사 완료, 정책 미확정, 구현 미착수** (2026-09-09)
>
...
| `reject` × `dlq` | 6 | 양쪽 다 격리 — **결과는 같지만 사유가 다르다**(§2-3) |
...
| fixture | 저장 | 감지 |
| --- | --- | --- |
| `speed_missing` | `PAYLOAD_VALIDATION_FAILED` | **no_alert** |
| `speed_null` | `PAYLOAD_VALIDATION_FAILED` | **no_alert** |
| `field_name_typo` (`sped`) | `UNKNOWN_FIELD` | **no_alert** |
| `gps_lat_missing` | `PAYLOAD_VALIDATION_FAILED` | no_alert (gps는 어느 룰에도 안 쓰인다) |
| `vehicle_id_missing` | `PAYLOAD_VALIDATION_FAILED` | **no_alert** |

원인은 `rules.py:34`의 `data.get(field)`다. 값이 없으면 `None`이고, `None`이면
**그 룰이 통째로 건너뛰어진다.** 예외도 로그도 지표도 없다.

**이게 왜 나쁜가**: 발행 측이 `speed`를 빠뜨리면 그 차량은 **과속 감지가 영원히 안 된다.**
저장은 거부되니 DLQ에서 보이지만, **감지 쪽은 완전히 조용하다.** "감지기가 도는데
알림이 없다"와 "감지기가 그 필드를 못 보고 있다"가 밖에서 구분되지 않는다.

`rpm_null_with_other_anomaly`가 이걸 제일 잘 보여준다 — `rpm`이 null이라 RPM 룰만
건너뛰고 엔진 과열은 정상적으로 잡힌다. **부분적으로 눈이 먼 채로 계속 돈다.**
...
| # | 질문 | 선택지 | 지금 기울어진 쪽 |
| ---: | --- | --- | --- |
| 1 | 필드 누락에 감지기는 어떻게 반응하나 | (a) 조용히 건너뛴다(현재) / (b) 지표를 올린다 / (c) DLQ | **(b)** — 조용한 것이 제일 나쁘고, DLQ는 과하다 |
...
| 6 | 공유 fixture를 누가 읽나 | (a) 양쪽 테스트 / (b) Python만 | **(a)** — 한쪽만 읽으면 갈라진다 |
...
**읽는 쪽은 아직 없다.** 정책을 정한 뒤에 만든다:
- `backend/src/test/java/com/telemetry/domain/SharedFixtureContractTest.java`
- `anomaly-detector/tests/test_shared_fixtures.py`

지금은 **현재 동작의 기록**이다. 기대값이 아니다.
