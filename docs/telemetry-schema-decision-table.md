...
### `timestamp` 형식이 계약으로 들어왔다 (2026-09-09, P0-2a)

예전에는 `@NotBlank`뿐이었고 형식은 **저장 단계**(`toPoint()`의 `Instant.parse`)에서
`DateTimeParseException`으로 터졌다. 감지 경로에는 그 단계가 아예 없어서 **같은 payload가
경로에 따라 다르게 끝났다.**

이제 `VehicleTelemetry.TIMESTAMP_PATTERN`이 계약이다 — ISO-8601에 **오프셋 필수**.
정규식은 모양만 보므로 `TelemetryDecoder`가 달력 검증(`Instant.parse`)을 한 번 더 한다.

**바뀐 것**: 잘못된 타임스탬프의 DLQ 사유가 `DateTimeParseException` →
`PAYLOAD_VALIDATION_FAILED`가 된다. 격리 단계가 저장에서 계약으로 앞당겨졌다.

**형식만 본다 — 신선도는 계약이 아니다.** 오래된 시각도 통과한다. 과거 데이터를 거부하면
DLQ 재처리와 백필이 불가능해진다.

어느 표준 라이브러리도 기준이 될 수 없다는 것을 실측했다 — 소문자 `t`/`z`는
Java가 받고 Python이 거부하며, 오프셋 없는 값과 공백 구분자는 그 반대다.
그래서 **정규식이 계약**이고 양쪽이 그걸 구현한다.

### 사유 선택 순서를 고정했다 (2026-09-09, P0-2a)
