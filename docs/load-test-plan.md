...
### Track A — 이상 감지 서비스 다중화 (3차 측정 8/2)

위에서 찾은 병목(`anomaly-detector-group` 단일 인스턴스)을 실제로 고치기 전에, 원인을
먼저 분리했다 — 24시간 테스트 당시엔 시뮬레이터 프로세스 6개(backend CPU 82-135%, kafka
35-119%)가 같은 8코어를 같이 썼기 때문에, "이상 감지 서비스 자체가 느린 것"과 "호스트를
뺏겨서 느려 보인 것"이 섞여 있었다.

**0단계 — 원인 분리(단일 인스턴스, 호스트 여유 있는 상태)**: 시뮬레이터 2프로세스만(200대/0.25초씩,
실측 ~1,200-1,250 msg/s 지속) 띄워 호스트에 여유를 남긴 채 12분 관찰.
...
Storage Lag은 전 구간 2-145로 건강. **결론**: ~1,200 msg/s에서는 단일 인스턴스도 평균적으론
따라잡는다(버스트성 진동만 있음) — 원래 24시간 테스트의 폭주(~2,500 msg/s)와는 확실히 다른
양상. 즉 진짜 처리 한계는 **~1,250 msg/s와 ~2,500 msg/s 사이 어딘가**에 있고, 호스트 경합이
그 한계를 넘었을 때의 악화 정도를 더 심하게 만들었을 가능성이 높다 — "순수 알고리즘 한계"와
"호스트 경합"이 둘 다 관여했다는 게 가장 정확한 설명이다.

**1단계 — 다중화**: `docker-compose.yml`의 `anomaly-detector` 서비스에서 `container_name`
고정을 제거(스케일과 고정 이름은 공존 불가). `docker compose up -d --scale anomaly-detector=3`
실행 후 `kafka-consumer-groups --describe`로 파티션 3개가 서로 다른 컨테이너 3개(IP 다름)에
정확히 1개씩 자동 배분되는 것을 확인. 로그에 컨테이너 hostname을 넣어 인스턴스 구분도 가능하게 함.

**2단계 — 재측정(원래 폭주를 냈던 부하로)**: 시뮬레이터 6프로세스(원래 테스트와 동일 설정)로
~2,324 msg/s를 11분 지속.
...
| 구성 | 부하 | Anomaly Lag 추이 | 판정 |
| --- | --- | --- | --- |
| 단일 인스턴스 | ~2,500 msg/s, 24시간 | 1,264 → 2,000,000+ (무한 발산) | 폭주 |
| **3인스턴스** | ~2,324 msg/s, 11분 | 16,438 → 20,690 → ... → **7,368**(피크가 시간이 갈수록 오히려 낮아짐) | **안정 — 발산 없음** |

같은 수준의 부하에서 단일 인스턴스는 무한정 쌓였지만, 3인스턴스는 11분 내내 수천~2만대에서
진동만 하고 발산하지 않았다(끝으로 갈수록 오히려 줄어듦). 24시간이 아니라 11분 관찰이라
"영원히 안 쌓인다"까지 단정하긴 이르지만, 원래 테스트의 발산 패턴은 단 몇 분 만에 뚜렷했던
것과 대조적으로 이번엔 그런 조짐이 전혀 없었다.

**참고**: 재빌드 과정에서 `anomaly-detector/requirements.txt`가 `kafka-python>=2.0.2`로
버전을 고정하지 않아, 1차 측정 때는 3.0.7, 이번엔 3.0.9가 설치됐다(재현성 관점의 사소한 흠 —
향후 과제로 버전 고정 고려). 인스턴스 시작 로그에 `kafka.net.selector`가 이벤트 루프 블로킹
경고를 1회 출력했지만 그룹 조인 시점 1회성이라 지속적인 처리 지연의 원인은 아닌 것으로 판단.
...
- [x] 이상 감지 서비스 원인 분리(호스트 경합 vs 알고리즘 한계) → 다중화(container_name 제거 + `--scale=3`) → 원래 폭주하던 부하로 재측정, 발산 없음 확인
...
- "Kafka 파티션 수(3)에 맞춰 컨슈머 concurrency를 1→3으로 조정 — 약한 부하와 실제 배압이
  ...
  두 번의 실측으로 증명한 사례."
- "찾아낸 병목(이상 감지 서비스 단일 인스턴스)을 실제로 고침 — Kafka Consumer Group은
  다중 인스턴스 리밸런싱을 기본 지원한다는 점에 착안해 인스턴스를 3개로 늘리고, 원래
  lag이 200만 건 이상으로 폭주했던 것과 같은 수준의 부하(~2,300 msg/s)로 다시 걸어봤다.
  단일 인스턴스는 무한정 쌓였지만 3인스턴스는 수천~2만 건대에서 진동만 하고 발산하지
  않았다 — 측정 → 병목 발견 → 개선 → 재측정을 끝까지 완결한 사례."
