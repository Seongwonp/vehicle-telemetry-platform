...
> `fdce9a3`(2026-08-04) 이후 수집 파이프라인이 초당 20건까지 무너져 있었다. 즉 이 soak에
...
#### 언제부터 무너졌나 — `fdce9a3`(2026-08-04, 보안/신뢰성 일괄 수정)
...
**중요한 건 안전장치를 되돌리지 않았다는 점이다.** `fdce9a3`이 지키려던 두 보장이 그대로다:
...
- **트레이드오프를 선택했으면 그 대가를 실제로 재야 한다.** `fdce9a3`은 트레이드오프를
