...
**확인 범위를 분명히 한다** — Prometheus API와 **Grafana 데이터소스 프록시로 쿼리를
실행**해서 결과를 받은 것이다. **브라우저에서 대시보드 화면을 열어 본 것은 아니다.**
패널이 눈으로 어떻게 그려지는지(범례 겹침, 축 스케일, 색)는 **미확인**이다.
...
### 6-1. `No data`를 "미발생"으로 읽지 않는다

실행에서 `실패·timeout 관찰`과 `발행 여부 불명`이 `No data`로 나왔다.
**그건 "해당 조회 조건에서 시계열이나 표본이 없다"는 뜻이지 미발생의 증거가 아니다.**

같은 `No data`가 나오는 원인이 여럿이다 — 실제 미발생, **수집 끊김**(`up == 0`),
**시계열 미생성**(그 라벨 조합으로 한 번도 안 오름), **조회 구간 문제**
(`rate()` 창보다 표본이 적음, 시간 범위 밖), 인스턴스 재기동으로 인한 초기화.

**미발생을 주장하려면 수집 상태를 같이 확인해야 한다** — `up{job=...}`,
Prometheus Targets, 그 지표가 한 번이라도 존재했는지.
쿼리 **실패**는 그것과 또 다르고 Grafana가 오류 배지로 표시한다.

### 6-2. 시도와 성공의 미세한 차이 — **원인 미확정**

실행에서 `발행 시도` rate가 0.04354, `성공 확인`이 0.04358이었다.
**왜 다른지 검증하지 않았다.** `rate()` 창 경계와 스크레이프 시점 차이로 보이지만
확인한 것이 아니다 — 카운터 자체는 같은 코드 경로에서 시도 직후 성공 시 함께 오른다.

**차이를 유실이나 미격리로 읽지 마라.** 원인을 알아야 한다면
같은 시각의 raw counter 값(`telemetry_kafka_dlq_publish_attempts_total`과
`_published_total`)을 직접 비교해야 하고, 그건 이번에 안 했다.
...
- **읽는 법** 텍스트 패널 — "시도 횟수이지 고유 메시지 수가 아니다", 단계별 지표 표,
  ...
  그리고 **`No data`는 미발생의 증거가 아니다**(수집 상태를 같이 봐야 한다).

**패널 JSON과 쿼리 실행은 확인했지만 브라우저 화면은 안 봤다** — 시각적 렌더링은 미확인이다.
- 계약 거부 판정 — 입구·사유별
...
- **브라우저에서 대시보드 화면을 보지 않았다.** 데이터소스 연결과 쿼리 실행은 Grafana
  API로 확인했지만, 패널이 눈으로 어떻게 그려지는지는 미확인이다.
- **시도/성공 rate의 미세한 차이는 원인 미확정**이다(§6-2).
