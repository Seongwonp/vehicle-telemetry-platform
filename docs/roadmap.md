...
| 3 | ~~Redis 장애 정책~~ **정책 완료(2026-09-12), 장애 실험은 남음** | 현재 Redis 장애가 rate limit 경로의 API 500과 refresh 중단을 만든다 | endpoint별 fail-open/fail-closed 결정 **완료** — 조회 fail-open / 진단·로그인 fail-closed / health 분리, 각 1회 무부하 측정으로 분기 확인. **30/90초 장애와 부하 중 측정은 미실행**: [`redis-failure-policy.md`](redis-failure-policy.md) §8 |
