...
P0-3에서 찾은 것. `init-topics.sh`가 만드는데 코드 어디서도 안 쓴다(DTC는 텔레메트리
payload 안에 있다). 제거 영향(모니터링·대시보드 참조)을 확인하고 지운다.

### P1.5-5. 반복 측정

P1-3·P1-5는 각 1회다. `docs/evidence-policy.md` 기준으로 안정성 주장에는 3회가 필요하다.
P1.5-2가 InfluxDB 장애의 2회차를 겸한다.
