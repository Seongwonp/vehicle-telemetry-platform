...
- ~~처리량 이득은 미측정~~ — **측정했다(2026-09-06 저녁)**. 파티션을 12로 올리고 Kafka에
  ...
  **새로 생긴 미검증**: 인스턴스 2개·6개 값, InfluxDB·호스트 CPU 중 무엇이 상한인지,
  발행을 더 올렸을 때의 천장, 3회 반복(증거가 남은 실행은 1회).
- **split 구성을 compose에 반영하지 않았다.** 기존 스크립트 수십 개가 `telemetry-backend`
  이름과 `localhost:8080`에 의존해서 별도 작업이다.
