...
| 3a | ~~Redis 장애 정책 구현·단일 장애 확인~~ **완료(2026-09-12)** | 현재 Redis 장애가 rate limit 경로의 API 500과 refresh 중단을 만든다 | 조회 fail-open / 진단·로그인 fail-closed / liveness·readiness 분리 구현. **무부하 단일 중지 각 1회**로 경로별 분기·503 구분·인증 유지 확인: [`redis-failure-policy.md`](redis-failure-policy.md) §8·§9 |
| 3b | Redis **부하 중 동작** — **미검증** (무부하 30/90초 반복은 완료) | fail-open은 통과시키되 Redis timeout(2초)까지 기다린다. 부하에서 이게 지연·스레드 점유로 번지는지 모른다 | 부하 중 p95/p99 지연과 Tomcat 스레드 점유. **무부하 30/90초 각 3회는 끝났다**(회복 +4~34초, 6회 모두 Lettuce 재연결 로그와 겹침 — 재연결 대기가 주요 후보, 원인 확정 아님 — [`RESULT`](../load-test/redis-outage/RESULT_20260913_recovery_repeat.md)). 90초 위상 효과 미배제 — 75·105초 측정은 보류. **2026-09-14 낮은 부하 L1 1회 — 무효**: 요청 발행이 계획 시각보다 최대 10.7 s 늦어 사전 기준(p95 ≤ 100 ms)을 넘었다 — 계획한 요청 간격을 지키지 못했다. 컨테이너 실행 지연이 부하 없이도 공통으로 관측됐고 Docker/가상화 계층을 의심하지만 **원인 미확정**. L2 미실행, 같은 환경에서 반복하지 않는다. 무효 실행의 응답 시간·스레드 수는 성능 결론으로 쓰지 않는다 — [`RESULT`](../load-test/redis-load/RESULT_20260914_L1.md) **조회 2초·진단 4초는 무부하 단건 관찰이지 고갈 근거가 아니다** — 회로 차단기는 이 측정 전에 추가하지 않는다 |
| 3c | ~~Redis **알림 지연**~~ **개선·검증 각 1회, 정책 결정(2026-09-13)** | 30·90초 장애 모두 복구 뒤 firing했다. 기존 규칙은 사실상 "최근 5분 이력"이었다 | 합계 카운터 기동 시 0 등록 + `[1m]` + `for: 1m` 유지. 90초 실패 중 firing·해제 약 1분. **"요청 처리에서 관찰한 Redis 영향" 알림으로 정의**(Redis 가용성 감시·지속 시간 측정 아님). **수용한 한계**: 30초 수준 장애 발송 0건(카운터·대시보드·로그로 조회, 이력 알림 추가 안 함) / Alertmanager 주기 유지 — 90초에서도 firing 수신이 Redis 응답 뒤, 앱 회복 뒤였던 실행 1/3. **백로그: 요청이 없을 때의 Redis 장애 탐지** — [`RESULT`](../load-test/redis-outage/RESULT_20260913_alert_timing.md) §6-4·§7-4 |
| 4 | 이벤트 상관관계 — **조사·설계 완료(2026-09-13), 구현 전 결정 대기** | HTTP traceId만으로 MQTT→Kafka→저장→알림 한 건을 잇기 어렵다 | 한 키로 로그·Kafka·DB·WebSocket을 조회. 설계: [`event-correlation-design.md`](event-correlation-design.md) — 1단계 `(vehicle_id, timestamp)` 표준화 / 2단계 optional `message_id`. **기존 anomaly `event_id`와 이름·공식을 섞지 않는다.** 2026-09-14 §10 보강 — ID 전달표(끊기는 곳은 계약 2곳·재직렬화 DTO 1곳), 같은 ID 다른 payload는 막지 않고 식별, 배포 순서(감지기 → 백엔드 `NON_NULL` → producer), 변경 파일·완료 조건. 결정 대기 5건(§10-7) |
...
### P0-2a. 이상 감지 경로의 입력 계약 — **완료(2026-09-09)**
...
**완료 조건**: (1) ~~실측표~~ (2) ~~공유 fixture 테스트~~ (3) ~~정책 확정~~ — **셋 다 완료**.

**정책: 공통 입력 계약을 유지한다.** 필수 센서 누락/null, 범위 밖, 타입 오류, unknown
필드는 **감지 이전에 거부**한다. 부분 감지는 도입하지 않았다 — 그건 기존 필수 입력 계약을
바꾸는 결정이고, 필요성이 확인되면 별도로 설계한다.
`speed: 300`은 과속 임계를 넘지만 **계약 밖이라 입력 오류**다. 201km/h는 차량 이상이다.

**결과**: fixture 61칸에서 **갈림 0** (적용 전 8칸). Java·Python이 61/61 같은 판정.
실제 파이프라인에서 사유 4종이 각각 격리되는 것과 Prometheus 지표가 오르는 것을 확인했다
(`evidence/20260909-P0-2a-detector/`).
**실제 Kafka에서 offset·재전달도 4/4 확인했다**
(`load-test/anomaly-contract-kafka/RESULT_20260909_kafka_contract.md`) — 가짜 Kafka 단위
테스트와 범위를 구분해서 기록했다. **발행 실패는 클라이언트 크기 제한으로 주입한 것이라
브로커 장애·timeout 복구·in-doubt는 검증 범위 밖이다.**
같은 원본 재처리에서 `event_id`가 동일한 것도 확인했고, DB 행·방송 멱등성은 기존 증거로 연결했다.
남은 갈래는 `docs/anomaly-path-contract.md` 6절.

#### 조사 결과 (2026-09-09) — `docs/anomaly-path-contract.md`

`contract-fixtures/cases.json` 21칸을 **양쪽 실제 코드로 재서** 갈리는 지점을 찾았다.

| 조합 | 칸 | 뜻 |
| --- | ---: | --- |
| 저장 `reject` × 감지 **`no_alert`** | **5** | 저장은 거부하는데 **감지는 조용히 지나간다** |
| 저장 `reject` × 감지 **`alert`** | **3** | **저장 안 된 데이터로 알림이 나간다** |
| 저장 `reject` × 감지 `dlq` | 6 | 둘 다 격리 — 결과는 같고 사유는 다르다 |
| 저장 `accept` × 감지 정상 | 7 | 설계대로 |

**제일 나쁜 것은 조용한 쪽이다.** `rules.py`의 `data.get(field)`가 None이면 그 룰이
통째로 건너뛰어진다 — 예외도 로그도 지표도 없다. 발행 측이 `speed`를 빠뜨리면
**그 차량은 과속 감지가 영원히 안 되는데 밖에서 알 방법이 없다.**

**조건부로 터지는 칸도 찾았다.** `vehicle_id` 누락은 **이상이 걸릴 때만**
`KeyError`로 DLQ에 간다. 같은 결함이 데이터에 따라 조용하기도, 격리되기도 한다.

**부수적으로 실제 결함을 하나 찾았다** — 감지기가 실제로 내는 예외
(`KeyError`/`TypeError`/`AttributeError`/`ValueError`)가 `dlq.py`에서 전부 `unknown`이다.
`KafkaTimeoutError`·`NoBrokersAvailable`은 **일시인데도** unknown이다.
저장 경로에서 두 번 한 실수인데 감지 경로는 아무도 안 봤다.
**고치지 않고** 현재 상태를 `test_dlq.py`에 고정했다 — 정책과 같이 결정한다.

**드리프트는 막아뒀다.** `contract-fixtures/cases.json`을 양쪽 테스트가 읽는다
(`SharedFixtureContractTest` 22건 / `test_shared_fixtures.py` 22건).
**정책 테스트가 아니라 특성화 테스트다** — 위 갈림을 그대로 고정한 것이고, 감지기
동작은 하나도 안 바꿨다. 정책이 정해지면 동작을 바꾸고 fixture 기대값을 같이 옮긴다.

만들면서 **검사가 안 도는 것**을 하나 잡았다 — fixture가 `backend/` 밖이라 Gradle이
입력으로 몰랐고, 기대값을 틀리게 바꿔놓고 돌렸는데 `:test UP-TO-DATE`로
BUILD SUCCESSFUL이 나왔다. `build.gradle`에 `inputs.file(...)`로 선언해 고쳤다.

**다음**: `anomaly-path-contract.md` 3-2절의 결정(특히 1번 필드 누락 처리,
4번 필수 필드 누락)을 확정한 뒤 구현한다.
