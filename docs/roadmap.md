...
| 4 | 이벤트 상관관계 — **조사·설계 완료(2026-09-13), 구현 전 결정 대기** | HTTP traceId만으로 MQTT→Kafka→저장→알림 한 건을 잇기 어렵다 | 한 키로 로그·Kafka·DB·WebSocket을 조회. 설계: [`event-correlation-design.md`](event-correlation-design.md) — 1단계 `(vehicle_id, timestamp)` 표준화 / 2단계 optional `message_id`. **기존 anomaly `event_id`와 이름·공식을 섞지 않는다.** 결정 대기 5건(§8) |
