...
있고 정적 멤버십은 그걸 피하려고 켠 것이다. **그 대가를 쟀다(2026-09-09)**: 같은 20초 정지에서 45,000ms는 리밸런싱 **0회**인데
10,000ms는 **2회**다(정지 중 멤버 6 → 3). 스케일 다운은 계획해서 할 수 있지만 GC 정지와
순단은 고를 수 없다 — **그래서 45,000ms를 유지한다.**
(`load-test/storage-scale/RESULT_20260909_session_timeout_cost.md`)
9초 밑으로 내리려면 `heartbeat.interval.ms`(기본 3초, 세션의 1/3 이하 권장)도 같이 봐야 한다.


### 정적 멤버십을 끈 1회에서 45초가 9초였다 — 그리고 폭풍 위험이 돌아온다
